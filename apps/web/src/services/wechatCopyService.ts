...
const getThemeInfo = () => {
  ...
  const currentTheme =
    ...
    state.allThemes.find((t) => t.id === themeId);
...
const svgMarkupToPng = async (svgMarkup: string): Promise<string> => {
  ...
  // 1. 背景透明（移除白色背景填充，以支持微信深色模式下的反色或背景融合）
  // 注意：如果是深色线条在深色背景下可能对比度不足，但为了响应"去除白色背景"的需求，我们保持透明
  ...
  for (const [index, block] of mermaidBlocks.entries()) {
    ...
    try {
      ...
      const targetGroups = Array.from(
        svgEl.querySelectorAll(selectors.join(", ")),
      );
      ...
      for (const g of targetGroups) {
        ...
        if (
          g.parentNode === svgEl &&
          (!refNode || refNode.parentNode === svgEl)
        ) {
          if (refNode) {
      ...
      const pngDataUrl = await svgMarkupToPng(fixedSvg);
